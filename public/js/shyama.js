...
(function () {
  ...
  function injectStyles() {
    ...
    var css = ""
      ...
      + "#sm-shyama{position:fixed;right:20px;bottom:166px;z-index:99998;pointer-events:none}"
      + ".sm-bubble{position:fixed;right:20px;bottom:92px;width:60px;height:60px;border-radius:50%;"
        + "background:linear-gradient(135deg,#f0b943,#3d7ef0);display:flex;align-items:center;justify-content:center;"
        + "font-size:27px;cursor:pointer;box-shadow:0 14px 38px rgba(30,64,120,.24),0 0 0 1px rgba(255,255,255,.55);"
        + "z-index:99999;border:1px solid rgba(255,255,255,.65);transition:transform .2s ease,box-shadow .2s ease;pointer-events:auto}"
      + ".sm-bubble:hover{transform:translateY(-3px) scale(1.045);box-shadow:0 18px 46px rgba(30,64,120,.30)}"
      + ".sm-bubble .dot{position:absolute;top:5px;right:5px;width:12px;height:12px;border-radius:50%;background:#19c979;border:2px solid #fff;box-shadow:0 0 0 3px rgba(25,201,121,.14)}"
      + ".sm-win{width:390px;max-width:calc(100vw - 32px);height:min(590px,calc(100dvh - 190px));min-height:320px;"
        + "background:rgba(255,255,255,.985);color:#13243b;backdrop-filter:blur(24px) saturate(140%);-webkit-backdrop-filter:blur(24px) saturate(140%);"
        + "border:1px solid rgba(36,73,120,.14);border-radius:22px;overflow:hidden;display:flex;flex-direction:column;"
        + "box-shadow:0 26px 80px rgba(24,54,94,.24),0 0 0 1px rgba(255,255,255,.65);opacity:0;visibility:hidden;"
        + "transform:translateY(12px) scale(.985);transform-origin:bottom right;pointer-events:none;"
      ...
      + ".sm-head{padding:14px 16px;display:flex;align-items:center;gap:12px;background:linear-gradient(135deg,#f8fbff,#eef5ff);border-bottom:1px solid rgba(36,73,120,.10)}"
      + ".sm-head .av{width:40px;height:40px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:22px;background:linear-gradient(135deg,#f0b943,#3d7ef0)}"
      + ".sm-head .t1{color:#13243b;font-weight:800;font-size:15px;line-height:1.1}"
      + ".sm-head .t2{color:#16885a;font-size:11px;display:flex;align-items:center;gap:5px;margin-top:3px}"
      + ".sm-head .t2 i{width:7px;height:7px;border-radius:50%;background:#19c979;display:inline-block;box-shadow:0 0 6px rgba(25,201,121,.45)}"
      + ".sm-head .x{margin-left:auto;color:#64748b;cursor:pointer;font-size:20px;line-height:1;border:0;background:transparent;padding:6px;border-radius:9px}"
      + ".sm-head .x:hover{background:rgba(36,73,120,.07);color:#13243b}"
      + ".sm-badge{display:flex;gap:6px;padding:8px 14px;background:#fbfdff;border-bottom:1px solid rgba(36,73,120,.09);flex-wrap:wrap}"
      + ".sm-chip{font-size:10.5px;padding:3px 9px;border-radius:20px;border:1px solid rgba(36,73,120,.13);color:#64748b;display:flex;align-items:center;gap:4px;background:#fff}"
      + ".sm-chip.ok{color:#16885a;border-color:rgba(25,201,121,.25);background:rgba(25,201,121,.07)}"
      + ".sm-body{flex:1;overflow-y:auto;padding:14px;display:flex;flex-direction:column;gap:10px;background:linear-gradient(180deg,#fff,#f8fbff)}"
      + ".sm-body::-webkit-scrollbar{width:6px}.sm-body::-webkit-scrollbar-thumb{background:rgba(36,73,120,.16);border-radius:3px}"
      + ".sm-row{display:flex;gap:8px;max-width:88%}.sm-row.user{align-self:flex-end;flex-direction:row-reverse}"
      + ".sm-row .ava{width:28px;height:28px;border-radius:50%;flex:0 0 28px;display:flex;align-items:center;justify-content:center;font-size:15px;background:linear-gradient(135deg,#f0b943,#3d7ef0)}"
      + ".sm-row.user .ava{background:linear-gradient(135deg,#dbe8f8,#b9cce6)}"
      + ".sm-msg{padding:9px 12px;border-radius:14px;font-size:13.5px;line-height:1.5;color:#334155;background:#fff;border:1px solid rgba(36,73,120,.11);white-space:pre-wrap;word-break:break-word;box-shadow:0 4px 14px rgba(36,73,120,.05)}"
      + ".sm-row.user .sm-msg{background:linear-gradient(135deg,#3d7ef0,#2a62cf);color:#fff;border-color:transparent}"
      + ".sm-quick{display:flex;flex-wrap:wrap;gap:7px;padding:0 14px 8px;background:#f8fbff}"
      + ".sm-quick button{font-size:12px;padding:6px 11px;border-radius:20px;cursor:pointer;color:#8a6518;background:rgba(240,185,67,.10);border:1px solid rgba(211,155,34,.26);transition:.15s}"
      + ".sm-quick button:hover{background:rgba(240,185,67,.18);transform:translateY(-1px)}"
      + ".sm-foot{padding:10px 12px;border-top:1px solid rgba(36,73,120,.10);display:flex;gap:8px;align-items:flex-end;background:#fff}"
      + ".sm-foot textarea{flex:1;resize:none;max-height:90px;background:#f8fbff;color:#13243b;border:1px solid rgba(36,73,120,.14);border-radius:12px;padding:9px 12px;font-size:13.5px;outline:none}"
      + ".sm-foot textarea:focus{border-color:rgba(61,126,240,.48);box-shadow:0 0 0 3px rgba(61,126,240,.08)}"
      + ".sm-foot textarea::placeholder{color:#8795a8}"
      + ".sm-send{width:40px;height:40px;flex:0 0 40px;border:0;border-radius:12px;cursor:pointer;color:#fff;font-size:16px;background:linear-gradient(135deg,#3d7ef0,#2a62cf)}"
      + ".sm-send:disabled{opacity:.5;cursor:not-allowed}"
      + ".sm-typing{display:flex;gap:4px;padding:10px 12px}.sm-typing span{width:7px;height:7px;border-radius:50%;background:#8795a8;animation:smb 1.2s infinite}"
      ...
      + "html.dark .sm-win{background:rgba(10,23,43,.985);color:#fff;border-color:rgba(255,255,255,.12);box-shadow:0 28px 90px rgba(0,0,0,.48)}"
      + "html.dark .sm-head{background:linear-gradient(135deg,rgba(18,43,82,.98),rgba(9,24,49,.98));border-color:rgba(255,255,255,.09)}"
      + "html.dark .sm-head .t1{color:#fff}html.dark .sm-head .x{color:#94a3b8}html.dark .sm-head .x:hover{background:rgba(255,255,255,.06);color:#fff}"
      + "html.dark .sm-badge,html.dark .sm-quick{background:rgba(8,20,39,.98);border-color:rgba(255,255,255,.08)}"
      + "html.dark .sm-chip{background:rgba(255,255,255,.04);border-color:rgba(255,255,255,.10);color:#94a3b8}"
      + "html.dark .sm-body{background:linear-gradient(180deg,#0d1b34,#071225)}"
      + "html.dark .sm-msg{background:rgba(255,255,255,.05);border-color:rgba(255,255,255,.10);color:#cbd5e1;box-shadow:none}"
      + "html.dark .sm-foot{background:#09172c;border-color:rgba(255,255,255,.09)}"
      + "html.dark .sm-foot textarea{background:rgba(255,255,255,.05);color:#fff;border-color:rgba(255,255,255,.11)}"
      + "@media(max-height:760px) and (min-width:481px){.sm-win{height:calc(100dvh - 176px);min-height:300px}#sm-shyama{bottom:156px}}"
      + "@media(max-width:480px){#sm-shyama{right:10px;left:10px;bottom:146px}.sm-win{width:auto;max-width:none;height:min(560px,calc(100dvh - 166px));min-height:280px}.sm-bubble{right:12px;bottom:78px;width:56px;height:56px;font-size:25px}}";
